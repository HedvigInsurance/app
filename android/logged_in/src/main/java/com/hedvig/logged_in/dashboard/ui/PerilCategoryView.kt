...
class PerilCategoryView : HedvigCardView {
    ...
    private val iconSize: Int by lazy { resources.getDimensionPixelSize(appR.dimen.dashboard_icon) }
    ...
    var categoryIconId: String? = null
        set(value) {
            ...
            @DrawableRes val icon = when (value) { // FIXME Oh god this is extremely wack
                "https://s3.eu-central-1.amazonaws.com/com-hedvig-web-content/du_och_din_familj%402x.png" -> appR.drawable.ic_family
                "https://s3.eu-central-1.amazonaws.com/com-hedvig-web-content/lagenhet%402x.png" -> appR.drawable.ic_home
                "https://s3.eu-central-1.amazonaws.com/com-hedvig-web-content/prylar%402x.png" -> appR.drawable.ic_things
                else -> appR.drawable.ic_family
            }
    ...
    private fun setupAttributes() {
        resources.getDimensionPixelSize(appR.dimen.base_margin).let { setPadding(it, it, it, it) }
        clipToPadding = true

        val attributes = context.obtainStyledAttributes(
            attributeSet,
            appR.styleable.PerilCategoryView,
            defStyle,
            0
        )

        attributes.getText(appR.styleable.PerilCategoryView_categoryUrl)?.let { url ->
            categoryIconUrl = Uri.parse(url.toString())
        }
        title = attributes.getText(appR.styleable.PerilCategoryView_title)
        subtitle = attributes.getText(appR.styleable.PerilCategoryView_subtitle)
