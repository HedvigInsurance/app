...
import com.hedvig.onboarding.react.ActivityStarterModule
...
class UploadBottomSheet : RoundedBottomSheetDialogFragment() {
    val chatViewModel: com.hedvig.onboarding.chat.ChatViewModel by sharedViewModel()
    ...
    private fun setupSubscriptions() {
        chatViewModel.isUploading.observe(this) { isUploading ->
            isUploading?.let { iu ->
                if (iu) {
                    dialog.header.text = resources.getString(appR.string.FILE_UPLOAD_IS_UPLOADING)
